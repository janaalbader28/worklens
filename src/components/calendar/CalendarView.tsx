...
export function CalendarView({
  ...
  const [cursor, setCursor] = useState(() => new Date(DEMO_TODAY.getFullYear(), DEMO_TODAY.getMonth(), DEMO_TODAY.getDate()));
  ...
  function goToday() {
    setCursor(new Date(DEMO_TODAY.getFullYear(), DEMO_TODAY.getMonth(), DEMO_TODAY.getDate()));
  }
...
function DayColumn({ date, items, expanded }: { date: Date; items: CalendarItem[]; expanded?: boolean }) {
  const isToday = date.toDateString() === DEMO_TODAY.toDateString();
  return (
...
function AddItemModal({
  ...
  const [date, setDate] = useState(DEMO_TODAY.toISOString().slice(0, 10));
