...
const AVAILABILITY_BUCKETS = [
  ...
  { key: "high", label: "High (> 25% available)" },
  { key: "medium", label: "Medium (10–25% available)" },
  { key: "low", label: "Low (< 10% available)" },
] as const;
...
export function TeamCapacityView({
  ...
  const filtered = useMemo(() => {
    return employees.filter((e) => {
      ...
      if (availability === "high" && avail <= 25) return false;
      if (availability === "medium" && (avail <= 10 || avail > 25)) return false;
      if (availability === "low" && avail >= 10) return false;
      return true;
    });
  }, [employees, query, skill, status, availability]);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <Select value={availability} onChange={setAvailability} label="Availability">
