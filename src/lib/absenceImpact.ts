...
function assessRisk(
  ...
  if (due.getTime() < DEMO_TODAY.getTime()) {
...
export function computeAffectedWork(
  ...
  tickets
    .filter((t) => (t.assignedEmployeeIds ?? []).includes(employee.id) && t.status !== "Resolved" && t.status !== "Closed")
    .forEach((t) => {
      const { risk, explanation } = assessRisk(t.expectedResolutionDate, start, end, t.estimatedHours, hoursPerDay);
      items.push({
        id: t.id,
        title: t.title,
        type: "Ticket",
        ...
        deadlineStatus: mapDeadlineStatus(t.expectedResolutionDate),
